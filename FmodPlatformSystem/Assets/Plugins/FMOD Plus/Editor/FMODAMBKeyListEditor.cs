...
namespace FMODPlus
{
    ...
    public class AmbKeyListEditor : Editor
    {
        ...
        private AMBKeyList _ambKeyList;
        ...
        private void OnEnable()
        {
            ...
            _ambKeyList = target as AMBKeyList;
            ...
            {
                Undo.RecordObject(_ambKeyList, "Create Clip");
                int count = 0;
            ...
            {
                Undo.RecordObject(_ambKeyList, "Remove Clip");
        ...
        {
            if (_ambKeyList.Clips.Count == 0)
                return;

            int count = 0;
            for (int i = 0; i < _ambKeyList.Clips.Count; i++)
                if (string.IsNullOrWhiteSpace(_ambKeyList.Clips.GetEventRef(i).Key))
                    count += 1;
            ...
            if (result)
            {
                var itemList = _ambKeyList.Clips.GetList();
        ...
        private void ShowDeleteToMultiKeyMessage()
        {
            ...
            for (int i = 0; i < _ambKeyList.Clips.Count; i++)
            {
                string key = _ambKeyList.Clips.GetEventRef(i).Key;
                int count = 0;
                for (int j = 0; j < _ambKeyList.Clips.Count; j++)
                    if (_ambKeyList.Clips.GetEventRef(j).Key == key)
                        count += 1;
