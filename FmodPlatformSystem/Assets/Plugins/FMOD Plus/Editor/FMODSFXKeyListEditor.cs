...
namespace FMODPlus
{
    ...
    public class SfxKeyListEditor : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            _sfxKeyList = target as SFXKeyList;
            ...
            {
                Undo.RecordObject(_sfxKeyList, "Remove Clip");
        ...
        {
            if (_sfxKeyList.Clips.Count == 0)
                return;

            int count = 0;
            for (int i = 0; i < _sfxKeyList.Clips.Count; i++)
                if (string.IsNullOrWhiteSpace(_sfxKeyList.Clips.GetEventRef(i).Key))
                    count += 1;
            ...
            if (result)
            {
                var itemList = _sfxKeyList.Clips.GetList();
        ...
        private void ShowDeleteToMultiKeyMessage()
        {
            ...
            for (int i = 0; i < _sfxKeyList.Clips.Count; i++)
            {
                string key = _sfxKeyList.Clips.GetEventRef(i).Key;
                int count = 0;
                for (int j = 0; j < _sfxKeyList.Clips.Count; j++)
                    if (_sfxKeyList.Clips.GetEventRef(j).Key == key)
                        count += 1;
